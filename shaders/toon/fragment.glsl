...
vec3 calculateDiffuse(vec3 lightDir, vec3 diffuseStrength, vec3 objColor)
{
	...
	float discrete = max(dot(norm, -lightDir), 0.0f);
	if(discrete < 0.33f) {
		discrete = 0.15f;
	} else if (discrete < 0.66f && discrete >= 0.33f) {
		discrete = 0.5f;
	} else {
		discrete = 0.8f;
	}
	return diffuseStrength * discrete * objColor;
}
...
vec3 calculateSpecular(vec3 viewPos, vec3 lightDir, vec3 specularStrength, vec3 objColor)
{
	...
	float discrete = max(dot(norm, halfwayDir), 0.0f);
	if(discrete < 0.33f) {
		discrete = 0.15f;
	} else if (discrete < 0.66f && discrete >= 0.33f) {
		discrete = 0.5f;
	...
	float spec = pow(discrete, material.shininess);
